...
BlocklyInterface.init = function() {
  ...
  BlocklyGames.coinval = BlocklyGames.getNumberParamFromUrl('Coinval', 0, 800);
  // alert(BlocklyGames.NAME);
  switch (BlocklyGames.NAME) {
    case 'start':
      BlocklyInterface.currentpage = 1;
      break;
    case 'circus':
      BlocklyInterface.currentpage = 2;
      break;
    case 'bridge':
...
BlocklyInterface.nextLevel = function() {
  // alert(BlocklyInterface.currentpage);
  // alert(BlocklyInterface.pages[BlocklyInterface.currentpage]);
  window.location = (BlocklyGames.IS_HTML ? BlocklyInterface.pages[BlocklyInterface.currentpage] : './') +
      '?lang=' + BlocklyGames.LANG+ '&level=' + (BlocklyGames.LEVEL) + '&Coinval=' + (BlocklyGames.coinval);
};
